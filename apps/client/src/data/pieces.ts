...
export function getPieceEmoji(id: string): string {
  return PIECES_MAP.get(id)?.emoji || "🚗";
}
...
export const FREE_PIECES = PIECES.filter((p) => p.cost === 0);
export const STORE_PIECES = PIECES.filter((p) => p.cost > 0);
